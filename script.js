...
function getMaxFlyBottom() {
    ...
    return Math.max(GROUND_BOTTOM, height - PLAYER_SIZE - 10);
}
...
function updateScore() {
    ...
    document.getElementById('current-score-display').textContent = `Score: ${score}  ·  Chunk: ${currentChunk()}/${MAP_CHUNKS}  ·  ${backStatus}  ${forwardStatus}`;
...
function maybeThrowHungrySkittle() {
    ...
    if (currentChunk() < THROW_START_CHUNK) return;
    if (thrownSkittles.length >= 6) return;
    throwCooldown -= 1;
    if (throwCooldown > 0) return;
    throwHungrySkittle();
    throwCooldown = THROW_EVERY_FRAMES;
}
...
function updateThrownSkittles() {
    ...
    thrownSkittles = thrownSkittles.filter(thrown => {
        thrown.worldX += thrown.vx;
        thrown.vy -= THROWN_GRAVITY;
        thrown.bottom += thrown.vy;
        if (thrown.bottom <= 20) {
            thrown.bottom = 20;
            thrown.vy *= -0.25;
            thrown.vx *= 0.92;
            if (Math.abs(thrown.vy) < 0.8) thrown.vy = 0;
        }
        if (thrown.bottom > maxBottom) {
            thrown.bottom = maxBottom;
            thrown.vy = 0;
        }
        ...
        if (screenX > container.clientWidth + 80 || (screenX < -100 && thrown.vx <= obstacleSpeed)) {
            thrown.el.remove();
            return false;
        }
        return true;
...
function isSkittleEater(el) {
    return Boolean(el && el.classList.contains('skittle'));
}
...
function initializeObstacles() {
    ...
    const skittleCount = 90;
    const minGap = safeRange;
    const maxGap = safeRange + 200;
...
function gameOver(collidedObstacle) {
    ...
    if (title) {
        if (collidedObstacle && collidedObstacle.classList.contains('chase-bird')) {
...
function updateInstructions() {
    ...
    if (window.innerWidth <= 768) {
        instructionsText.innerHTML = `
            <h2>How to Play</h2>
            <p>Hold the screen to fly. Let go to fall</p>
            ...
            <p>At chunk 5, the giant Skittle throws hungry Skittles at you</p>
            <p>Don't touch the birds</p>
            <p>Score increases with each successful jump</p>
            <p>Reach the finish line at chunk 20</p>
        ...
        instructionsText.innerHTML = `
            <h2>How to Play</h2>
            <p>Hold W, SPACE, or UP to fly. Let go to fall</p>
